...
Reading OpenAI's [Agent SDK](https://openai.com/index/introducing-agents-sdk/), I kept having a feeling I couldn't place. Then it clicked: I'd seen this pattern before. Not once — three times.

The first was [the centrifugal governor](https://en.wikipedia.org/wiki/Centrifugal_governor) in the 1780s. Before it, a worker stood next to the steam engine adjusting the valve by hand. After it, a weighted flyball mechanism sensed rotational speed and adjusted the valve automatically. The worker didn't disappear. The job changed: from turning the valve to designing the governor.

The second was [Kubernetes](https://kubernetes.io/). You declare desired state — three replicas, this image, these resource limits. A controller continuously observes actual state. When they diverge, the controller reconciles: restarts crashed pods, scales replicas, rolls back bad deployments. The engineer's job shifted from restarting services to writing the spec the system reconciles against.
...
[Claude Code](https://openai.com/index/claude-code/): 100% AI-generated, zero written by hand. They call it "harness engineering."

**Same pattern each time.**

Norbert Wiener named it in 1948: **cybernetics**, from the Greek κυβερνήτης — steersman. You stop turning the valve. You steer.
...
[Karpathy](https://x.com/karpathy): "The future of work is AI engineers designing environments and feedback loops for other AI agents." He had 16 parallel agents build a C compiler, but carefully designed test infrastructure. "Most of my effort went into designing the environment around Claude — the tests, the environment, the feedback."
...
The work is making your judgment machine-readable. Architecture docs that describe actual layering and dependency direction. Custom linters with remediation instructions baked in. Golden principles that encode your team's taste.

[OpenAI](https://openai.com/): they spent 20% of every Friday cleaning up "AI slop" — until they encoded their standards into the harness itself.
...
The generation-verification asymmetry — the intuition behind [LLM-as-Judge](https://arxiv.org/abs/2306.05685), by Cobbe et al. — points to where this goes. Generating a correct solution is harder than verifying one. You don't need to out-implement the machine. You need to out-evaluate it: specify what "correct" looks like, recognize when the output misses, judge whether the direction is right.

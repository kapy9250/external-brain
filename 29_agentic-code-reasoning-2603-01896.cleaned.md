...
Can an LLM agent explore a codebase and determine whether two patches are semantically equivalent, without ever running them? We study this capability, which we call _agentic code reasoning_: an agent’s ability to navigate files, trace dependencies, and gather context iteratively to perform deep semantic analysis without executing code. This capability is essential for tasks like bug detection, code review, and patch verification in complex repositories where relevant context spans multiple files. However, evaluating whether an agent’s reasoning is _correct_ presents a fundamental challenge: how do we establish ground truth for code understanding? And how do we ensure agents reason thoroughly rather than guess?
...
We introduce _semi-formal reasoning_, a general approach that bridges this gap. Rather than training specialized models or formalizing semantics, we prompt agents with structured reasoning templates that require explicit evidence for each claim. These templates act as _certificates_: the agent must state premises, trace relevant code paths, and provide formal conclusions. The structured format naturally encourages interprocedural reasoning, as tracing program paths requires the agent to follow function calls rather than guess their behavior.
...
Error Analysis for Verifier With Tests. We closely analyzed the remaining errors for Opus-4.5 with Semi-formal and found three primary failure modes:

1.   1.Incomplete execution tracing despite the given instructions: The agent assumed function behavior without fully tracing concrete execution paths. 
2.   2.Third-party library semantics: The agent guessed behavior from function names when source code was unavailable. 
3.   3.Dismissing subtle differences: The agent identified semantic differences but incorrectly concluded they were irrelevant to test outcomes. 
...
1.   1.Indirection bugs: The bug is in a class not directly invoked by the test. For example, in Csv_12, the test calls CSVParser.parse() but the bug is in CSVFormat.withHeader()—a configuration class the model consistently overlooks. 
2.   2.Multi-file bugs: Bugs spanning multiple files require identifying all locations. Large ground-truth sets (5+ lines across 2+ files) are systematically harder. 
...
_Task:_ Determine if two patches that fix 2-digit year formatting for years << 1000 produce identical test outcomes.

_Ground Truth:_ NOT EQUIVALENT (Patch 1 calls wrong format function causing AttributeError)

Standard reasoning (INCORRECT) concluded:

> _“Both patches fix the same bug. Patch 1 uses format(self.data.year, ’04d’)[-2:] which pads to 4 digits and takes the last 2, while Patch 2 uses modulo 100 and pads to 2 digits. For year 476: Patch 1 gives ’0476’[-2:] = ’76’ and Patch 2 gives ’76’. Both produce identical results.”_
...
Mockito_8 involves a StackOverflowError caused by infinite recursion in Java generic type resolution. The bug is in GenericMetadataSupport.java. When processing a self-referential type like SelfRef<T extends SelfRef<T>>, the method registerTypeVariablesOn at line 80 overwrites a safe T → TypeVarBoundedType(T) mapping with a self-referential T → T, causing getActualTypeArgumentFor to recurse infinitely. The ground-truth fix is at line 79.
...
The agent verified this does _not_ happen for non-self-referential cases like UpperBoundedTypeWithClass<E extends Number & Comparable<E>>: there, registerTypeVariablesOn(Comparable<E>) maps T_Comparable→\to E, but T_Comparable≠\neq E (different declarations), so E’s existing BoundedType mapping is preserved.
...
Without Phase 2’s systematic code path tracing, the agent in standard mode stopped at the crash site, since getActualTypeArgumentFor is where the StackOverflowError manifests and is the obvious answer. The semi-formal template forced the agent to trace the full registration pipeline (O1–O10), test multiple type scenarios, formulate competing hypotheses (H1–H3), and ultimately discover that the overwrite at line 80 is the root cause, not the recursive method that crashes. The agent explicitly tested SelfRef<T extends SelfRef<T>> against UpperBoundedTypeWithClass<E extends Number & Comparable<E>> to confirm why self-referential types uniquely trigger the bug, a differential analysis that standard mode never attempted.
...
1.   1.Locate and read both function implementations 
2.   2.Document their exact signatures and behavior in the trace table 
3.   3.Identify semantic differences with specific file:line evidence 
4.   4.Check whether the opposite conclusion is supported by evidence 
...
_Question:_ “Is there a difference between using m_sliceTypeStrMap.at(m_sliceType) vs m_sliceTypeStrMap[m_sliceType]?”
...
_Question:_ “Do we need the check for ‘if not build_directory’?”

The correct answer is that the check is _not_ needed because the Runner constructor already handles this case. However, semi-formal reasoning (57% score) produced:
...
The agent thoroughly traced five functions and found a real edge case where BUILD_DIR could be empty. However, it _missed_ that Runner’s constructor also checks build_directory (indirectly through find_fuzzer_path, which returns None if build_directory is falsy, triggering a CorpusPruningException). This illustrates how deeper reasoning can lead to _more confident wrong answers_ when the agent follows a plausible-but-incorrect chain without verifying all downstream code paths.

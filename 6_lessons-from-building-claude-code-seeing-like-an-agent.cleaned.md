...
Claude acts through Tool Calling, but there are a number of ways tools can be constructed in the Claude API with primitives like bash, skills and recently code execution (read more about programmatic tool calling on the Claude API in the official docs).
...
(you can read more about why we made an ExitPlanTool in the related writeup)

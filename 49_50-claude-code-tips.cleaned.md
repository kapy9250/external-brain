...
Run `source ~/.zshrc` to load it. Now you type `cc` instead of `claude`, and you skip every permission prompt. The flag name is intentionally scary. Only use it after you fully understand what Claude Code can and will do to your codebase. I covered this and more aliases in another post.
...
Claude runs the tests, sees failures, and fixes them without you stepping in. For UI changes, set up Playwright so Claude can open a browser, interact with the page, and verify the UI works as expected. That feedback loop catches issues that unit tests miss.
...
The GitHub CLI (`gh`) handles PRs, issues, and comments without a separate MCP server. CLI tools are more context-efficient than MCP servers because they don't load tool schemas into your context window. Same applies to `jq`, `curl`, and other standard CLI tools.
...
`/effort` is a keyword that sets effort to high and triggers adaptive reasoning on Opus 4.6. Claude dynamically allocates thinking based on the problem. Use it for architecture decisions, tricky debugging, multi-step reasoning, or anything where you want Claude to think before acting.
...
Skills are markdown files that extend Claude's knowledge on demand. Unlike instructions which load every session, skills load only when relevant to the current task. This keeps your context lean.
...
Run `claude remote-control` to start a session, then connect to it from the browser or the Claude app on iOS/Android. The session runs locally on your machine. The phone or browser is just a window into it. You can send messages, approve tool calls, and monitor progress from anywhere.
...
The Claude Code team calls this Agent Teams. Spin up 3-5 worktrees, each running its own Claude session in parallel. I usually run 2-3. Each worktree gets its own session, its own branch, and its own file system state.

The ceiling on local worktrees is your machine. Multiple dev servers, builds, and Claude sessions all competing for CPU. Claude Containers moves each agent to its own cloud container with a browser preview, so your machine stays free for the work that needs your brain.
...
Your main session stays clean with plenty of room to build something. A deep investigation can consume half your context window before you write any code. Subagents keep that cost out of your main session. Built-in types include Explore (Haiku, fast file search) and Plan (read-only analysis).
...
When context compacts (automatically or via `/compact`), tell Claude what to preserve: "`/compact` focus on the API changes and the list of modified files." You can also add standing instructions to your settings: "When compacting, preserve the full list of modified files and current test status."
...
Use `@` to reference files directly: `@middleware.ts` has the session handling. The `@` prefix resolves to the file path automatically, so Claude knows exactly where to look.
...
The output tends to be bloated. If you can't explain why a line is there, delete it. Trim the noise and add what's missing.
...
Over time your `CLAUDE.md` becomes a living document shaped by real mistakes. To keep it from growing indefinitely, use file references to reference a separate file like `rules.md` for patterns and fixes. Your `CLAUDE.md` stays lean, and Claude reads the details on demand.
...
Reference docs with `@api.md`. You can also reference `@types.md`, `@schema.json`, or even `@~/.claude/my-project-instructions.md`.

Claude reads the file when it needs it. Think of it as "here's more context if you need it" without bloating the file Claude reads every session.
...
Different from using subagents on the fly, custom subagents are pre-configured agents saved in `.claude/agents/`. For example, a security-reviewer agent with Opus and read-only tools, or a quick-search agent with Haiku for speed.
...
Claude Code supports dynamic tool loading, so servers only load their definitions when Claude needs them.
...
`/branch` (or `/fork`) creates a copy of your conversation at the current point. Try the risky refactor in the branch. If it works, keep it. If it doesn't, your original conversation is untouched. This is different from rewind because both paths stay alive.
...
First Claude implements the feature, second Claude reviews. The reviewer has no knowledge of the implementation shortcuts and will challenge every one of them.
